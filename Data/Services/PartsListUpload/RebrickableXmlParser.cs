...
public sealed class RebrickableXmlParser : IPartsListFormatParser
{
    public PartsUploadFormat Format => PartsUploadFormat.RebrickableXml;
    ...
    public (List<RawRow> Rows, List<InvalidRow> InvalidRows, HashSet<int> InvalidColorIds) Parse(string content)
    {
        ...
        // Rebrickable-style inventories are commonly:
        ...
        foreach (var item in itemElements)
        {
            ...
            // Typical tags:
            // ITEMTYPE, ITEMID, COLOR, QTY, MINQTY, ALTERNATE, EXTRA
            ...
            var colorStr = GetValue(item, "COLOR") ?? GetValue(item, "COLORID") ?? GetValue(item, "COLOR_ID");
            var qtyStr = GetValue(item, "QTY") ?? GetValue(item, "MINQTY") ?? GetValue(item, "QUANTITY");

            if (string.IsNullOrWhiteSpace(partNum))
            {
                invalidRows.Add(new InvalidRow(null, null, 0, $"Item {lineNo}: Missing ITEMID/PartNum"));
                continue;
            }

            if (!TryParseInt(colorStr, out var colorId))
            {
                invalidRows.Add(new InvalidRow(partNum, null, 0, $"Item {lineNo}: Invalid COLOR '{colorStr}'"));
                continue;
            }

            if (!TryParseInt(qtyStr, out var qty) || qty <= 0)
            {
                invalidRows.Add(new InvalidRow(partNum, colorId, 0, $"Item {lineNo}: Invalid QTY '{qtyStr}'"));
                continue;
            }

            rows.Add(new RawRow(partNum.Trim(), colorId, qty, ColorMode: "Rebrickable"));
        }
