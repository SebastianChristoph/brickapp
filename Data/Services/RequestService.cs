...
namespace brickapp.Data.Services
{
        ...
        NotificationService notification)
    {
        ...
        public async Task<NewItemRequest> CreateNewItemRequestAsync(
            string brand,
            string name,
            string userId,
            string? partNum,
            IBrowserFile? imageFile = null)
        {
            ...
            logger.LogInformation(
                "🟡 [RequestService] CreateNewItemRequestAsync called: Brand={Brand}, Name={Name}, UserId={UserId}, PartNum={PartNum}, ImageFileNull={ImageFileNull}",
            ...
            {
                logger.LogInformation(
                    "🟡 [RequestService] Image file provided. Brand={Brand}, PartNum={PartNum}, Uuid={Uuid}",
                    brand, partNum, request.Uuid);

                if (brand.ToLower().Trim() == "lego" && !string.IsNullOrEmpty(partNum))
                {
                    await imageService.SaveResizedItemImageAsync(imageFile, brand, partNum, null);
                }
                else
                {
                    await imageService.SaveResizedItemImageAsync(imageFile, brand, null, request.Uuid);
                }
            }
            ...
            logger.LogInformation(
                "🟡 [RequestService] NewItemRequest created with Id={Id} and Uuid={Uuid}",
            ...
            bool nameChanged = false)
        {
            ...
            if (string.IsNullOrWhiteSpace(partNum))
            {
                ...
                existingBrick = brand switch
                {
                    ...
                    _ => await db.MappedBricks.FirstOrDefaultAsync(m => m.UnknownName == request.Name)
                };
            }
            else
            {
        ...
        public async Task<bool> IsNewItemRequestBlockedAsync(string name, string brand)
        {
            ...
            return await db.NewItemRequests.AnyAsync(r =>
                r.Name == name &&
                r.Brand == brand &&
        ...
        public async Task<bool> UpdateNewSetRequestAsync(
            int id,
            string brand,
            string setNo,
            string setName,
            string? imagePath,
            List<NewSetRequestItem> items,
            NewSetRequestStatus status)
        {
            await using var db = await dbFactory.CreateDbContextAsync();
        ...
        public async Task<NewSetRequest> CreateNewSetRequestAsync(
            string brand,
            string setNo,
            string setName,
            string? imagePath,
        ...
        public async Task ApproveNewSetRequestAsync(int requestId)
        {
            ...
            if (request == null || request.Status != NewSetRequestStatus.Pending)
                return;
        ...
        public async Task<MappingRequest> CreateMappingRequestAsync(
            int brickId,
            string brand,
            string mappingName,
            ...
            string userId)
        {
            ...
            {
                await db.SaveChangesAsync();
                logger.LogInformation("MappingRequest created: {@request}", request);
                return request;
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Failed to create MappingRequest: {@request}", request);
                notification.Error($"Failed to create MappingRequest: {ex.Message}");
                throw;
            }
        }
        ...
        public async Task ApproveMappingRequestAsync(int requestId, string adminUserId)
        {
            ...
            if (request == null || request.Status != MappingRequestStatus.Pending)
                return;
            ...
            if (brick != null)
            {
                ...
                {
                    case "Lego":
                        brick.LegoName = request.MappingName;
                        brick.LegoPartNum = request.MappingItemId;
                        break;

                    case "BlueBrixx":
                        brick.BluebrixxName = request.MappingName;
                        brick.BluebrixxPartNum = request.MappingItemId;
                        break;

                    case "Cada":
                        brick.CadaName = request.MappingName;
                        brick.CadaPartNum = request.MappingItemId;
                        break;

                    case "Pantasy":
                        brick.PantasyName = request.MappingName;
                        brick.PantasyPartNum = request.MappingItemId;
                        break;

                    case "Mould King":
                        brick.MouldKingName = request.MappingName;
                        brick.MouldKingPartNum = request.MappingItemId;
                        break;

                    case "Unknown":
                        brick.UnknownName = request.MappingName;
                        brick.UnknownPartNum = request.MappingItemId;
                        break;
                }

                brick.HasAtLeastOneMapping = true;
                logger.LogInformation("🟡 [RequestService] HasAtLeastOneMapping = true für {Name} gesetzt", brick.Name);
            }
        ...
        public async Task<ItemImageRequest> CreateItemImageRequestAsync(
            int mappedBrickId,
            string userId,
            IBrowserFile imageFile)
        {
            ...
            if (string.IsNullOrWhiteSpace(brick.Uuid))
            {
                ...
                await db.SaveChangesAsync();
                logger.LogInformation("Generated UUID {Uuid} for MappedBrick {BrickId}", brick.Uuid, brick.Id);
            }

            // Bild in pending/ speichern
            var pendingImagePath = await imageService.SaveResizedItemImageAsync(
                imageFile,
                "pending",
                null,
                brick.Uuid);
            ...
            logger.LogInformation(
                "ItemImageRequest created with Id={Id} for MappedBrick={BrickId}, image stored at {Path}",
                request.Id, mappedBrickId, pendingImagePath);
        ...
        public async Task ApproveItemImageRequestAsync(int requestId, string adminUserId)
        {
            ...
            {
                logger.LogError("ItemImageRequest {RequestId} has no valid brick or UUID", requestId);
                return;
            }

            // Bild von pending/ nach new/ verschieben
            var finalPath = await imageService.MoveImageAsync(
                request.TempImagePath,
                "new",
                null,
                brick.Uuid);
            ...
            {
                logger.LogError("Failed to move image for ItemImageRequest {RequestId}", requestId);
                return;
            }
            ...
            logger.LogInformation(
                "ItemImageRequest {RequestId} approved and image moved from pending to new: {FinalPath}",
                requestId, finalPath);
        }
        ...
        public async Task RejectItemImageRequestAsync(int requestId, string adminUserId, string reason)
        {
            ...
            {
                await imageService.DeleteImageAsync(request.TempImagePath);
                logger.LogInformation(
                    "Deleted pending image at {Path} for rejected request {RequestId}",
                    request.TempImagePath, requestId);
            }
            ...
            logger.LogInformation("ItemImageRequest {RequestId} rejected", requestId);
        }
        ...
        public async Task RejectItemImageRequestToPendingAsync(int requestId, string adminUserId, string reason)
        {
            ...
            logger.LogInformation(
                "ItemImageRequest {RequestId} set back to pending with reason",
                requestId);
        }
    }
}
